...
class Tree(Transform):
    ...
    def delimited(self, file, delimiter='\n', bufsize=4096):
        buf = ''
        while True:
    ...
    def load(self, data, lang=None, max_len=None, **kwargs):
        ...
        if isinstance(data, str) and os.path.exists(data):
            ...
            with open(data, 'r', encoding='utf8') as f:
                ...
                # debug only
                # for iid, s in enumerate(d):
                #     nltk.Tree.fromstring(s)
                #     print(iid)

                # trees = [nltk.Tree.fromstring(s) for s in d]
                trees = []
                for idx, s in enumerate(d):
                    ...
                    try:
                        ...
                        print("ERROR trees.append in IDX :", idx)
                        print("s :", s)
                        print("Exiting...")
                        sys.exit(0)
            ...
            if lang is not None:
                ...
                data = [data] if isinstance(data[0], str) else data0
